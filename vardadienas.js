(function () {
  ...
  function renderNameDay() {
    const el = document.getElementById('namedayList');
    if (!el) return;
    const key = getTodayKey();
    const list = namedays[key] || [];
    if (list.length === 0) {
      el.textContent = "Nav ierakstu šai dienai";
      return;
    }
    // show as comma-separated with underline
    el.innerHTML = "<u>" + list.join(', ') + "</u>";
  }

  // initial render and hourly refresh (matches page refresh policy)
  renderNameDay();
  // also update when system date changes
  setInterval(renderNameDay, 60 * 1000); // refresh every minute
})();